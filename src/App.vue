<template>
  <div class="sp-root" itemscope itemtype="https://schema.org/LocalBusiness">
    ...
    <transition name="sp-modal-fade">
      <div v-if="showSuccessModal" class="sp-modal-overlay" @click.self="showSuccessModal = false" role="dialog"
        aria-modal="true" aria-labelledby="sp-modal-title">
        <div class="sp-modal">
          ...
          <h3 id="sp-modal-title">Заявка отправлена!</h3>
          <p>Мы ответим вам в течение пары часов. Ожидайте звонка или письма на указанные контакты.</p>
          <button @click="showSuccessModal = false" class="sp-btn-blue sp-modal-close-btn">Отлично, спасибо!</button>
        </div>
      </div>
    </transition>
    ...
    <header :class="['sp-header', { scrolled: isScrolled }]" role="banner">
      ...
      <div id="sp-mobile-menu" class="sp-mmenu" :class="{ open: menuOpen }" role="navigation">
        ...
        <a href="https://wa.me/79252753031" target="_blank" rel="noopener noreferrer" class="sp-btn-wa sp-btn-wa-mob">
          <svg viewBox="0 0 24 24" fill="currentColor" width="18" height="18">
            <path
              d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm4.64 6.8c-.15 1.58-.8 5.42-1.13 7.19-.14.75-.42 1-.68 1.03-.58.05-1.02-.38-1.58-.75-.88-.58-1.38-.94-2.23-1.5-.99-.65-.35-1.01.22-1.59.15-.15 2.71-2.48 2.76-2.69.01-.03.01-.14-.06-.2-.07-.06-.18-.04-.26-.02-.12.02-2.01 1.28-2.83 1.8-.27.18-.51.27-.73.27-.24 0-.7-.14-1.04-.25-.44-.15-.86-.32-1.24-.48-.41-.17-.45-.41-.07-.65 1.44-.94 3.15-1.87 5.13-2.8 2.44-1.14 2.95-1.34 3.28-1.34.07 0 .23.02.33.13.09.09.12.21.13.29.01.09.02.22-.01.34z" />
          </svg>
    ...
    <main>
      ...
      <section id="contact" class="sp-sec sp-sec-white" aria-labelledby="contact-title">
        <div class="sp-container">
          ...
          <div class="sp-form-wrapper sp-animate sp-fade-up" style="--delay: 100ms">
            <form @submit.prevent="submitForm" class="sp-contact-form" novalidate>
              ...
              <div class="sp-form-group">
                <label for="email">Email <span class="sp-optional">— необязательно</span></label>
                <input type="email" id="email" v-model="form.email" placeholder="your@email.com" autocomplete="email" />
              </div>
              <div class="sp-form-group">
                <label for="message">Сообщение <span class="sp-optional">— необязательно</span></label>
...
const handleKeydown = (e) => { if (e.key === 'Escape' && showSuccessModal.value) showSuccessModal.value = false }
...
onMounted(() => {
  ...
  if (!document.querySelector('meta[name="viewport"]')) {
    ...
    meta.content = 'width=device-width, initial-scale=1.0, maximum-scale=5.0'; document.head.appendChild(meta)
  }
  ...
  setMeta({ name: 'keywords', content: 'перегородки из ПГП, газобетонные перегородки, межкомнатные перегородки Москва, кладка перегородок, ПГП под ключ, газобетон под ключ, шумоизоляция перегородок, перегородки Ytong, перегородки Knauf, пазогребневые плиты монтаж' })
  ...
  setMeta({ property: 'og:image:alt', content: 'Монтаж межкомнатных перегородок — СтеныПро Москва' })
  ...
  if (!document.querySelector('script[type="application/ld+json"]')) {
    ...
    addSchema({
      ...
      name: 'СтеныПро',
      description: 'Возводим межкомнатные перегородки из ПГП и газобетона в Москве и МО. До 50 м² за 1 день.',
      ...
      hasOfferCatalog: {
        '@type': 'OfferCatalog', name: 'Услуги по монтажу перегородок',
        itemListElement: [
          { '@type': 'Offer', name: 'Монтаж перегородок из ПГП (80 мм)', price: '1030', priceCurrency: 'RUB', unitText: 'м²' },
          { '@type': 'Offer', name: 'Кладка газобетонных перегородок (75–100 мм)', price: '930', priceCurrency: 'RUB', unitText: 'м²' },
          { '@type': 'Offer', name: 'Шумоизоляция стен', price: '300', priceCurrency: 'RUB', unitText: 'м.п.' }
        ]
      },
      aggregateRating: { '@type': 'AggregateRating', ratingValue: '5', reviewCount: '3', bestRating: '5' },
      review: [
        { '@type': 'Review', author: { '@type': 'Person', name: 'Алексей Смирнов' }, reviewBody: 'Ребята за 3 дня возвели все стены. Лазером проверял сам — отклонений нет.', reviewRating: { '@type': 'Rating', ratingValue: '5' } },
        { '@type': 'Review', author: { '@type': 'Person', name: 'Елена Викторовна' }, reviewBody: 'Мастера работали очень аккуратно, весь мусор в мешки собрали. Рекомендую.', reviewRating: { '@type': 'Rating', ratingValue: '5' } },
        { '@type': 'Review', author: { '@type': 'Person', name: 'Дмитрий' }, reviewBody: 'Цену зафиксировали сразу, лишнего не накручивали. Рекомендую.', reviewRating: { '@type': 'Rating', ratingValue: '5' } }
      ]
    })
    addSchema({
      '@context': 'https://schema.org', '@type': 'FAQPage',
      mainEntity: [
        { '@type': 'Question', name: 'Нужно ли штукатурить ваши стены?', acceptedAnswer: { '@type': 'Answer', text: 'Да, штукатурка требуется для идеальной геометрии, но благодаря нашей точной кладке слой будет минимальным (5-10 мм). ПГП (пазогребень) можно просто шпаклевать.' } },
        { '@type': 'Question', name: 'Как делаете перемычки над дверями?', acceptedAnswer: { '@type': 'Answer', text: 'Для перемычек мы используем металлический уголок толщиной 3 мм или закладную арматуру, что гарантирует прочность.' } },
        { '@type': 'Question', name: 'Вывозите ли вы мусор?', acceptedAnswer: { '@type': 'Answer', text: 'Сбор мусора в мешки и уборка рабочего места входят в стоимость. Вывоз и заказ контейнера оплачиваются отдельно.' } }
      ]
    })
    addSchema({ '@context': 'https://schema.org', '@type': 'BreadcrumbList', itemListElement: [{ '@type': 'ListItem', position: 1, name: 'Главная', item: 'https://возведениеперегородок.рф/' }] })
  }
...
watch(showSuccessModal, (val) => { document.body.style.overflow = val ? 'hidden' : '' })

const scrollTo = (hash) => { document.querySelector(hash)?.scrollIntoView({ behavior: 'smooth' }) }

const submitForm = async () => {
  if (!form.value.name || !form.value.phone) {
    formMessage.value = 'Заполните имя и телефон'; formMessageType.value = 'error'; return
  }
  ...
  try {
    await emailjs.send(EMAILJS_SERVICE, EMAILJS_TEMPLATE, {
      ...
      email: form.value.email || 'не указан',
      message: form.value.message || 'не указано',
    })
...
.sp-modal-fade-enter-active .sp-modal,
.sp-modal-fade-leave-active .sp-modal {
  transition: transform 0.25s ease, opacity 0.25s ease;
}

.sp-modal-fade-enter-from,
.sp-modal-fade-leave-to {
  opacity: 0;
}

.sp-modal-fade-enter-from .sp-modal {
  transform: scale(0.92) translateY(16px);
}

.sp-modal-fade-leave-to .sp-modal {
